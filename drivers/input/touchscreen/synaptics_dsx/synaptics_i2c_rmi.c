		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (data->proximity_data[0] == 0x00) {
		...
		if (f51->finger_is_hover) {
			tsp_debug_dbg(true, &rmi4_data->i2c_client->dev, "Hover[OUT]\n");
	...
	if (data->finger_hover_det && (data->hover_finger_z > 0)) {
		...
		if (!f51->finger_is_hover) {
			...
			tsp_debug_dbg(true, &rmi4_data->i2c_client->dev, "Hover[IN]"
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
				" x = %d, y = %d, z = %d\n", x, y, z);
#else
				"\n");
...
static int synaptics_rmi4_f51_set_general_control_2(struct synaptics_rmi4_data *rmi4_data)
{
	...
	tsp_debug_dbg(true, &rmi4_data->i2c_client->dev, "%s: [0x%02X]\n",
...
static int synaptics_rmi4_check_status(struct synaptics_rmi4_data *rmi4_data)
{
	...
	tsp_debug_dbg(true, &rmi4_data->i2c_client->dev, "%s: Device status[0x%02x] status.code[%d]\n",
	...
	if (status.flash_prog == 1) {
		...
		tsp_debug_dbg(true, &rmi4_data->i2c_client->dev, "%s: In flash prog mode, status = 0x%02x\n",
...
flash_prog_mode:
	...
	tsp_debug_dbg(true, &rmi4_data->i2c_client->dev, "%s: Product ID [%s. %d]\n",
		__func__, rmi4_data->rmi4_mod_info.product_id_string, rmi4_data->product_id);
	tsp_debug_dbg(true, &rmi4_data->i2c_client->dev, "%s: Result, date, revision, version, pr_num, b/l [%d, %02d/%02d, 0x%02X, 0x%02X, PR%d, %c%c[%c.%c]]\n",
	...
	for (ii = 0; ii < rmi4_data->num_of_intr_regs; ii++) {
		if (rmi4_data->intr_mask[ii] != 0x00) {
			tsp_debug_dbg(true, &rmi4_data->i2c_client->dev, "%s: Interrupt enable[%d] = 0x%02x\n",
...
static void synaptics_rmi4_f51_finger_timer(unsigned long data)
{
	...
	if (!rmi4_data->fingers_on_2d) {
		...
		if (rmi4_data->f51->finger_is_hover) {
			tsp_debug_dbg(true, &rmi4_data->i2c_client->dev, "Hover[OUT][Timer]\n");
...
static int synaptics_rmi4_init_exp_fn(struct synaptics_rmi4_data *rmi4_data)
{
	...
	list_for_each_entry(exp_fhandler, &rmi->exp_fn_list, link) {
		if (exp_fhandler->initialized) {
			tsp_debug_dbg(true, &rmi4_data->i2c_client->dev, "%s: [%d] is already initialzied.\n",
					__func__, exp_fhandler->fn_type);
			continue;
		}
...
static int synaptics_rmi4_setup_drv_data(struct i2c_client *client)
{
	...
	if (pdata->get_ddi_type) {
		...
		tsp_debug_dbg(true, &client->dev, "%s: DDI Type is %s[%d]\n",
...
static void synaptics_rmi4_sensor_sleep(struct synaptics_rmi4_data *rmi4_data)
{
	...
	tsp_debug_dbg(true, &rmi4_data->i2c_client->dev, "%s, from %s\n",
