...
unsigned int mem00_msg_cnt;
unsigned int mem01_msg_cnt;
unsigned int mem10_msg_cnt;
unsigned int mem11_msg_cnt;

#define PRINT_LITER	500
...
static struct nocp_cnt *check_get_cnt_done(struct list_head *target_list, struct nocp_cnt *nocp_cnt)
{
	...
	unsigned long long tmp_byte;
	unsigned long long tmp_cycle;
	...
	list_for_each_entry(tmp_nocp, target_list, list) {
		...
		if (tmp_nocp->id == MEM0_0) {
			if (mem00_msg_cnt > PRINT_LITER) {
		...
		if (tmp_nocp->id == MEM0_1) {
			if (mem01_msg_cnt > PRINT_LITER) {
				tmp_byte = ((cnt_val1 << 16) | cnt_val0) * 100;
				tmp_cycle = ((cnt_val3 << 16) | cnt_val2) * 16;
				do_div(tmp_byte, tmp_cycle);
				pr_debug("$$$$$$$$$ %s BYTE[%d]CYCLE[%d] Ratio[(byte/cycle * 16) *100] : %d\n",
		...
		if (tmp_nocp->id == MEM1_0) {
			if (mem10_msg_cnt > PRINT_LITER) {
				tmp_byte = ((cnt_val1 << 16) | cnt_val0) * 100;
				tmp_cycle = ((cnt_val3 << 16) | cnt_val2) * 16;
				do_div(tmp_byte, tmp_cycle);
				pr_debug("$$$$$$$$$ %s BYTE[%d]CYCLE[%d] Ratio[(byte/cycle * 16) *100] : %d\n",
		...
		if (tmp_nocp->id == MEM1_1) {
			if (mem11_msg_cnt > PRINT_LITER) {
				tmp_byte = ((cnt_val1 << 16) | cnt_val0) * 100;
				tmp_cycle = ((cnt_val3 << 16) | cnt_val2) * 16;
				do_div(tmp_byte, tmp_cycle);
				pr_debug("$$$$$$$$$ %s BYTE[%d]CYCLE[%d] Ratio[(byte/cycle * 16) *100] : %d\n",
